...
{
    public partial class frmCajaIngreso : Form
    {
        public frmCajaIngreso()
        {
            InitializeComponent();
        }
        private void button3_Click(object sender, EventArgs e)
        {
            if (checkBox1.Checked)
            {
                ...
                {
                    if (int.Parse(txtNtraspaso.Text) > 0)
                    {
                        claseBD cbd = new claseBD();
                        string updatefrm = "update frmcajaingreso set frm_abierto = 'si' , frm_variable_valor = " + txtNtraspaso.Text + " where frm_id = 1";
                        cbd.EjecutarIUD(updatefrm);
                        frmIngresoMercaderia fim = new frmIngresoMercaderia();
                        fim.Show();
                        this.Close();
                    }
                }
                ...
                {
                    MessageBox.Show("INGRESE EL DETALLE PARA GENERAR EL INGRESO DE MERCADERIA");
                }
            }
            else
            {
        ...
        {
            string CadSql222;
            CadSql222 = "Select bol_numero as bol_id from boleta order by bol_id DESC LIMIT 1";
        ...
        private void cmdSalir_Click(object sender, EventArgs e)
        {
            if (dgvLista.Rows.Count == 0)
            {
                ...
                claseBD cbd = new claseBD();
                string eliminarcaja = "truncate table caja_ingreso";
                cbd.EjecutarIUD(eliminarcaja);
                frmGestionProductos gpr = new frmGestionProductos();
                gpr.Show();
                this.Close();
        ...
        public double total2 = 0,precioVenta = 0, precioCosto = 0, cantcaptura = 0, totalcost = 0, stock = 0, resstock = 0, sumstock = 0;
        public string CadSql;                    
        private double cant = 0;
        private double cant2 = 0;
        private void ingresorapido(KeyPressEventArgs e)
        {

            claseBD cbd3 = new claseBD();
            string truncatecajaing = "truncate table caja_ingreso";
            string insertcajafact = "insert into caja_ingreso (nombre,cod_interno,cod_barra,precio,cant,total,costo,margen)" +
            "select b.trd_nombre_producto,b.trd_codigo_interno,b.trd_codigo," +
            "b.trd_precio_producto,b.trd_cant,trd_valor_total," +
            "b.trd_valor_total,b.trd_margen from traspasos_detalle b where b.tra_numero = " + txtNtraspaso.Text + " ";
            cbd3.EjecutarIUD(truncatecajaing);
            cbd3.EjecutarIUD(insertcajafact);
            claseBD.conex.Close();
            llenarDgvCaja();
            //INSTERTAR EN CAJA_INGRESO
        }
        private void ingresogrdcaja(KeyPressEventArgs e)
        {
            ...
            {       
                    ...
                    else
                    {
                        ...
                        try
                        {
                            string consultacaja = "select * from caja_ingreso where nombre = '" + lblNombreprodCaptura.Text + "' ";
                            CBd.AbrirConexion();
                       ...
                       if (Codinterno == txtCodigoBarra.Text || codBarra == txtCodigoBarra.Text.Substring(0,7))
                        {
                            ...
                            if (lblNombreprodCaptura.Text == lblconsultacaja.Text)
                            {
                                ...
                                string updatecaja = "Update caja_ingreso Set cod_barra = '" + lblCodBarraCaptura.Text + "',precio = '" + lblPrecioCostoCaptura.Text + "',cant = '" + Convert.ToDouble(rescant) + "',total = '" + totales.ToString() + "',costo = '" + rescost + "' , margen = '" + lblmargen.Text + "' where nombre = '" + lblconsultacaja.Text + "' ";
                                CBd.EjecutarIUD(updatecaja);
                                //MessageBox.Show(updatecaja.ToString());
                                string updatestock = "update productos set Stock = '" + sumstock.ToString() + "' where nombre_prod = '" + lblconsultacaja.Text + "' ";
                            ...
                            {
                                string cadsql3 = "insert into caja_ingreso values('" + lblNombreprodCaptura.Text + "','" + lblCodInternoCaptura.Text + "','" + lblCodBarraCaptura.Text + "','" + lblPrecioCostoCaptura.Text + "','" + Convert.ToDouble(cantcaptura) + "','" + totalcost + "','" + totalcost + "','" + lblmargen.Text + "')";
                                CBd.EjecutarIUD(cadsql3);
                                string updatestock = "update productos set Stock =  '" + sumstock.ToString() + "' where nombre_prod = '" + lblNombreprodCaptura.Text + "' ";
                                CBd.EjecutarIUD(updatestock);
                                llenarDgvCaja();
                                txtCodigoBarra.Clear();
                                lblconsultacaja.Text = "";
                            ...
                            foreach (DataGridViewRow row in dgvLista.Rows)
                            {
                                ...
                                costo += Convert.ToDouble(row.Cells["total"].Value);
        ...
        private void frmGuiaDespacho_Load(object sender, EventArgs e)
        {
            ...
            msksubtotal.Text = Decimal.Round( Convert.ToDecimal(subtotal),0).ToString();
            ...
            resultado = iva + sub;
            txtTotal.Text = "" + resultado;
            lblprecioculture.Text = Convert.ToInt32(Decimal.Round( Convert.ToDecimal( msksubtotal.Text.ToString()),0)).ToString("c", CultureInfo.CreateSpecificCulture("es-CL"));
            ...
            try
            {
                ...
                try
                {
                    ...
                    while (Rec.Read())
                    {
                        ...
                        resstock = double.Parse(stockstring) - cantidaddecimal;
                    }
                }
                catch (Exception ex)
                {
                ...
                string updatecaja = "delete from caja_ingreso where cod_barra = '" + codBarra + "'";
                CBd.EjecutarIUD(updatecaja);
       ...
       private void txtCodigoBarra_KeyPress_1(object sender, KeyPressEventArgs e)
       {
        ...
        //string updatestock = "update productos set Stock = '" + sumstock.ToString() + "' where nombre_prod = '" + nombreupd + "' ";
        private void txtNtraspaso_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == Convert.ToChar(Keys.Enter))
            {
                if (dgvLista.RowCount == 0)
                ...
                {
                    MessageBox.Show("EL DETALLE DEBE ESTAR VACIO, PARA CARGAR PRODUCTOS TRASPASO");
                }
            }
        ...
        private void txtNtraspaso_TextChanged(object sender, EventArgs e)
        {
