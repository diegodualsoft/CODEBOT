...
namespace Prototipo1
{
    public partial class frmFactEmitidas : Form
    {
        ...
        {
            int rest = 0, a = 0, b = 0, positivo = 0;

            if (cbmediopago.Text != "Seleccione...." && txtneto.Text != "0" && txtiva.Text != "0" && txttotal.Text != "0" && txtmontocaja.Text != "")
            {
                a = int.Parse(txtrestante.Text);
                b = int.Parse(txtrestante.Text);
                rest = int.Parse(txtrestante.Text);
                if (rest >= 0)
                {
                ...
                {
                    positivo = Math.Abs(positivo+a);
                    if (MessageBox.Show("Cuadratura realizada correctamente con sobrantes de caja en valor = $" + positivo.ToString(), "Advertencia", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        string delete = "delete from emision where nfact = '" + numfact.Text + "'";
                        claseBD cbd = new claseBD();
                        cbd.EjecutarIUD(delete);
        ...
        {
            frmGestionComercial a = new frmGestionComercial();
            a.Show();
            this.Close();
        }
